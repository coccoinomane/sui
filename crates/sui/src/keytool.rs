...
use fastcrypto_zkp::bn254::zk_login::{AuxInputs, PublicInputs, ProofPoints, big_int_str_to_hash};
    ...
    pub fn execute(self, keystore: &mut Keystore) -> Result<(), anyhow::Error> {
        match self {
            ...
            KeyToolCommand::GenerateZkLoginAddress { sub_id_com, iss } => {
                ...
                println!("Sui Address: {:?}", SuiAddress::from_bytes(hasher.finalize().digest)?);
            }
            ...
            KeyToolCommand::SerializeZkLoginAuthenticator {
                ...
                let foundation_key: SuiKeyPair = SuiKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
